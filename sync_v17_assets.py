...
for filepath in all_htmls:
    ...
    # Bump css/js cache busters to v=17.0
    if is_blog:
        content = re.sub(r'href="[^"]*styles\.css[^"]*"', 'href="../css/styles.css?v=17.0"', content)
        content = re.sub(r'src="[^"]*main\.js[^"]*"', 'src="../js/main.js?v=17.0"', content)
    else:
        content = re.sub(r'href="[^"]*styles\.css[^"]*"', 'href="css/styles.css?v=17.0"', content)
        content = re.sub(r'src="[^"]*main\.js[^"]*"', 'src="js/main.js?v=17.0"', content)
...
print(f"Updated cache buster tags (v=17.0) across {len(all_htmls)} HTML files!")
